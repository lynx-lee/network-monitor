...
![Version](https://img.shields.io/badge/version-v3.4.2-blue)
...
- **README 文档更新**：补充 V3.4.1 版本日志，新增版本 badge，更新最后修改日期

### V3.4.1

- **设备卡片默认展开**：`compactNodes` 默认值从 `true` 改为 `false`，用户初次使用时不再困惑于折叠的卡片内容
- **代码审查修复**：全面修复代码审查发现的 P0-P3 级别问题
- **WebSocket 重连修复**：`reconnect()` 使用正确的 `listeners` 属性名，保留自定义事件监听器
- **Express 静态文件服务兼容**：生产模式下 `app.get('*')` 改为 `app.use()` 中间件方式，兼容新版 `path-to-regexp`
- **Harbor 镜像发布**：后端和前端 Docker 镜像推送至 Harbor (192.168.124.21)，支持 `3.4.1` 和 `latest` 标签
- **CT 部署支持**：服务部署至 PVE CT 1025 (192.168.124.25)，使用 tsx 运行时运行编译后代码
...
*Last updated: 2026-07-19 (V3.4.2)*

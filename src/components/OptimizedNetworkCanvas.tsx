...
import NetworkDeviceNode from './OptimizedNetworkDeviceNode';
...
const OptimizedNetworkCanvas: React.FC = () => {
    ...
    return (connections || []).map((conn) => {
      ...
      if (sourceDevice && targetDevice) {
        ...
        if (sourcePort && targetPort) {
          // Line rate is minimum of two port rates
  ...
  // 使用拖拽优化钩子
  const { onNodesChange } = useDragOptimization({
  ...
  useEffect(() => {
    ...
    // This ensures that built-in toggle interactivity button shows correct state
    storeApi.setState({
...
const MemoizedOptimizedNetworkCanvas = React.memo(OptimizedNetworkCanvas);

export default MemoizedOptimizedNetworkCanvas;
